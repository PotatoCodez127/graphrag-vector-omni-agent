...
def build_org_chart_graph():
    ...
    # Entities
    employees = ["Sarah", "David", "Aisha"]
    skills = ["Rust", "Python", "React", "Cloud Architecture"]
    departments = ["Backend", "Frontend", "DevOps"]

    # Add Nodes
    for emp in employees: G.add_node(emp, type="Employee")
    for skill in skills: G.add_node(skill, type="Skill")
    for dept in departments: G.add_node(dept, type="Department")
...
# Initialize it once so it stays in memory
